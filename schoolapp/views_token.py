...
def build_agora_token(channel_name, uid):
    ...
    expiration_ts = int(
        (
            ...
            + timedelta(seconds=TOKEN_EXPIRATION_SECONDS)
        ).timestamp()
    )
    ...
    if user.user_type == "3":  # Student
        try:
            ...
            if room.classroom_id != student.class_id_id:
                return JsonResponse(
                    {"error": "Access denied to this classroom room."}, status=403
                )
        except Students.DoesNotExist:
            return JsonResponse({"error": "Student profile not found"}, status=403)

    elif user.user_type == "2":  # Staff
        if not room.classroom_id and not room.staff:
            return JsonResponse(
                {"error": "Access denied to this staff room."}, status=403
            )

    elif user.user_type == "1":  # Admin
        pass  # Admin can access any room
